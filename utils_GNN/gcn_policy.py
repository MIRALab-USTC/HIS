...
class GCNPolicy(torch.nn.Module):
    ...
    def forward(self, pivot_node_features, edge_indices, children_features):
        ...
        # print("gcn pivot_node_features", pivot_node_features)
        children_features = self.children_node_embedding(children_features)
        # print("gcn children_features", children_features)
        children_features = self.conv_v_to_c(pivot_node_features, edge_indices, children_features)
        pivot_node_features = self.conv_c_to_v(children_features, reversed_edge_indices, pivot_node_features)
        # print("gcnC pivot_node_features", pivot_node_features)
        # print("gcnC children_features", children_features)
        ...
        if self.mean_max is not None:
            ...
            if attention is None:
                if self.mean_max == 'max':
                    ...
                    scores = np.mean(scores, axis=1, keepdims=True) + np.std(scores, axis=1, keepdims=True)
                elif self.mean_max == 'mean':
                    scores = np.mean(scores, axis=1, keepdims=True)
            else:
                scores = scores * attention
                scores = np.sum(scores, axis=1, keepdims=True)        
        ascending_indexes = np.argsort(scores.squeeze()) # default ascending order 
...
class GCNMultiHeadPolicy(GCNPolicy):
    ...
    def get_output(self, pivot_node_features):
        output = [cur_module(pivot_node_features) for cur_module in self.output_module]
